...
```
###### Question 1
In the code below, change `geom_point()` to `geom_jitter()`. Adjust the size to `2`, the alpha to `0.5`, and set the height of the jitter to `0.2`.  Note how this changes the plot.
...
```
###### Question 5
Copy one of your plots from above, and inside the `aes()` function, set `size=` to a numeric variable not already represented on the plot. Keep in mind that you'll need to remove the `size=` *attribute* from `geom_jitter()` for the points to size correctly. Be sure to also adjust the legend labels. What new information do you gain about Hampshire County bridges from the plot?
